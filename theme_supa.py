...
import db
from db import COMMON_THEME_SET
        ...
        if workbook is not None and db_url:
            with st.spinner("Syncing Excel structure to Supabase..."):
                try:
                    ...
                    quarter_id_map = db.sync_workbook_structure(conn, workbook)
            ...
            with st.expander(f"{ticker} — {len(items)} quarters", expanded=True):
                ...
                if workbook is None:
                    ...
                    missing_periods = [
                        ...
                        if db.period_to_quarter(item["period"]) not in quarter_id_map[ticker]
                    ]
                    if missing_periods:
                        st.error(
                            f"No row found for: {', '.join(missing_periods)} "
                            f"(expected column A format like '{db.period_to_quarter(items[0]['period'])}')"
                        )
                    else:
                        st.success(
                            "Matched rows: "
                            + ", ".join(
                                f"{item['period']} -> id {quarter_id_map[ticker][db.period_to_quarter(item['period'])]}"
                                for item in items
                            )
                        )
                        group["existing_themes"] = db.get_existing_variable_themes(cur, ticker)
                        group["db_ready"] = True
